...
public final class HudAPIManager {
	...
	static void renderWorldHud(GuiGraphics context, DeltaTracker tickCounter) {
	...
	private static void drawWorldLine(GuiGraphics context, Minecraft client, String label, String value, int line, int valueColor) {
		context.pose().pushMatrix();
		context.pose().translate(WORLD_X, lineOffset(client, line));
		context.pose().scale(WORLD_HUD_SCALE, WORLD_HUD_SCALE);
		String prefix = label + ": ";
		context.drawString(client.font, prefix, 0, 0, COLOR, true);
		context.drawString(client.font, value, client.font.width(prefix), 0, valueColor, true);
